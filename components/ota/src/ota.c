...
static int image_header_get(ota_image_info_t* info)
{
    ...
    ret = ota_mgr_image_read(OTA_IMAGE_ADDRESS, cache_buf, OTA_HEADER_SIZE);
    if(ret != 0)
    {
...
static bool image_data_check(const ota_image_info_t* info)
{
    ...
    while (remaining)
    {
        ...
        {
            ota_mgr_image_read(OTA_IMAGE_ADDRESS+offset+OTA_HEADER_SIZE, cache_buf, WRITE_BLOCK);
        ...
        {
            ota_mgr_image_read(OTA_IMAGE_ADDRESS+offset+OTA_HEADER_SIZE, cache_buf, remaining);
...
static int image_write_app(const ota_image_info_t* info)
{
    ...
    while (remaining)
    {
        if(remaining >= WRITE_BLOCK)
        {
            ret = ota_mgr_image_read(OTA_IMAGE_ADDRESS+offset+OTA_HEADER_SIZE, cache_buf, WRITE_BLOCK);
            if(ret != 0)
            {
        ...
        {
            #if CONFIG_OTA_IMAGE_AES128_ENCRYPT
                ret = ota_mgr_image_read(OTA_IMAGE_ADDRESS+offset+OTA_HEADER_SIZE, cache_buf, align_length(remaining,16));
                if(ret != 0)
                {
                    mlog_e("read ota_image partition error, 0x%x", offset+OTA_HEADER_SIZE);
                    return OTA_FLASH_ERR;
            ...
            #else
                ret = ota_mgr_image_read(OTA_IMAGE_ADDRESS+offset+OTA_HEADER_SIZE, cache_buf, remaining);
                if(ret < 0)
                {
                    mlog_e("read ota_image partition error, 0x%x", offset+OTA_HEADER_SIZE);
                    return OTA_FLASH_ERR;
...
{
    int ret = ota_mgr_image_erase(OTA_IMAGE_ADDRESS, OTA_IMAGE_REGION_SIZE);
    if (ret == 0)
        return 0;
    else
        return -1;
